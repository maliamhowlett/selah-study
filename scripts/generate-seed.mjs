// Generates supabase/seed-malia.sql from the hardcoded class data.
...
// Data mirrored from src/lib/classes.ts (Malia's Fall 2026 classes).
const CLASSES = [
  {
    slug: "acct-20000",
    ...
    gradingScale:
      "A 90–92, A>92 | B 83–86, B+ 87–89 | C 73–76, C+ 77–79 | D 63–66, D+ 67–69 | F <60. Curve considered only if class avg < 75%.",
    keyDates: [
    ...
    latePolicy:
      "Late assignments are NOT accepted. No extensions for expired Connect trials/inactive licenses.",
    aiPolicy:
      "AI permitted for non-exam assignments as a study tool. NOT allowed on the 3 exams. Do not upload instructor materials or student data.",
    gradeInquiryPolicy:
  ...
  },
  {
    ...
    gradingScale:
      "A 93.5–100 | A- 90–93.49 | B+ 88–89.99 | B 84–87.99 | B- 81–83.99 | C+ 78–80.99 | C 76–77.99 | C- 70–75.99 | D 60–69.99 | F 0–59.99",
    keyDates: [
      {
        label: "Course Units",
        date: "Gilgamesh → Hebrew Bible → Odyssey → Prometheus Bound → Othello → Research Paper",
      },
    ],
    attendance:
    ...
    latePolicy:
      "Not explicitly stated; assumed strict given discussion-based format.",
    aiPolicy:
  ...
  },
  {
    ...
    additionalStaff: [
      {
        role: "Alt. Lecturer (Sec. 200)",
        name: "Dr. Allan Bickle",
    ...
    meetingTimes: [
      {
        ...
        time: "3:30–4:20 PM",
        location: "CL50 224",
        note: "Lecture — Section 100",
      },
      {
        days: "T/R",
        time: "7:30 AM",
        location: "TBD",
        note: "Recitation with TA (quizzes here!)",
      },
    ],
    overview:
    ...
    gradingScale:
      "A+ ≥97 | A ≥93 | A- ≥90 | B+ ≥87 | B ≥83 | B- ≥80 | C+ ≥77 | C ≥73 | C- ≥70 | D+ ≥67 | D ≥60. Cutoffs may drop but never rise.",
    keyDates: [
    ...
    latePolicy:
      "Late HW not accepted; no makeup quizzes. Missed exam requires notifying lecturer within 24 hours.",
    aiPolicy:
      "AI OK for learning/understanding topics. Cannot use AI answers as your own. No AI on exams (no electronics allowed).",
    gradeInquiryPolicy:
      "Contact your TA first for most questions (deadlines, grading, technical). TA escalates to instructor if needed.",
    notes: [
  ...
  },
  {
    ...
    grading: [
      {
        item: "Prepare + Attend + Participate (15 pts/day × 26 classes)",
        weight: "390 pts",
      },
      { item: "5 Quizzes (10 pts each)", weight: "50 pts" },
      { item: "2 Exploration Activities (50 pts each)", weight: "100 pts" },
      {
        item: "Student Interview + Career Interview (50 pts each)",
        weight: "100 pts",
      },
      {
        item: "2 Reflection Papers (Midterm + Final, 50 pts each)",
        weight: "100 pts",
      },
      {
        item: "5 Assignments & Instructor Choice (25/50/75 pts each)",
        weight: "260 pts",
      },
    ],
    gradingScale: null,
    ...
    latePolicy:
      "Late assignments accepted up to 24 hrs late with a 20% point penalty. Submit as PDF only.",
    aiPolicy:
      "AI is NOT an appropriate tool for assignments in this class. Use your own voice/perspective.",
    gradeInquiryPolicy: null,
    notes: [
  ...
  },
  {
    ...
    textbooks: [
      "No required textbook. Readings posted to Brightspace as needed.",
    ],
    grading: [
      { item: "Attendance: Lecture", weight: "6%" },
      { item: "Attendance: 1:1 meetings", weight: "5%" },
      {
        item: "Attendance: Insurance Company Presentations (2 required)",
        weight: "3%",
      },
    ...
    keyDates: [
      ...
      {
        label: "Final Exam",
        date: "Finals week — do NOT book winter break travel before knowing date",
      },
    ],
    attendance:
